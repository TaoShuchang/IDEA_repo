...
def main(opts):
    ...
    for i in seeds:
        ...
        setup_seed(i)
        print('----------- Seed ',i, '-----------')
        rgcn = RGCN(nnodes=features.shape[0], nfeat=features.shape[1], nclass=labels_np.max()+1,
                    nhid=32, device=device)
        rgcn.load_state_dict(torch.load(save_file + f'_pert0.00_seed' + str(i) + '_checkpoint.pt'))
        rgcn = rgcn.to(device)
        adj_tensor, rgcn.features, rgcn.labels  = utils.to_tensor(adj, features.todense(), labels_np, device=device)
        
        rgcn.adj_norm1 = normalize_tensor(adj_tensor, power=-1/2)
        rgcn.adj_norm2 = normalize_tensor(adj_tensor, power=-1.0)


        logits = rgcn.predict()
        # logits = rgcn.predict(feat, adj_tensor)
        val_acc = accuracy(logits[val_mask], labels[val_mask])
        test_acc = accuracy(logits[test_mask], labels[test_mask])
        tar_acc = accuracy(logits[test_mask], labels[test_mask])
        graph_name_arr.append('Clean')
        acc_tar_arr.append(test_acc)
        ...
        for graph in graph_save_file:
            ...
            new_adj, new_features, new_labels= utils.to_tensor(new_adj, new_features.todense(), new_labels_np, device=device)
        
            rgcn.adj_norm1 = normalize_tensor(new_adj, power=-1/2)
            rgcn.adj_norm2 = normalize_tensor(new_adj, power=-1)
...
'''
nohup python -u test_rgcn.py --dataset ogbarxiv --suffix rgcn --atk_suffix seed123 --gpu 0 > log/ogbarxiv/evasion_rgcn.log 2>&1 & 
...
nohup python -u test_rgcn.py --dataset citeseer --suffix rgcn --atk_suffix seed123 --gpu 2 > log/citeseer/evasion_rgcn_new.log 2>&1 & 
nohup python -u test_rgcn.py --dataset cora --suffix rgcn --atk_suffix seed123 --gpu 4 > log/cora/evasion_rgcn.log 2>&1 & 
