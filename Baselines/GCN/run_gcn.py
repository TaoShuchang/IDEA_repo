...
def main(opts):
    ...
    for pert in np.arange(0, 0.3, 0.05):
        ...
        for i in seeds:
            setup_seed(i)
            print('----------- Seed ',i, '-----------')
            gcn = GCN(nfeat=features.shape[1], nhid=args.hid, nclass=labels.max().item() + 1, lr=args.lr, weight_decay=args.decay, dropout=0.5, device=device)
            gcn = gcn.to(device)
            start_time = time.time()
            gcn.fit(features, adj, labels_np, train_mask, val_mask, test_mask, train_iters=2000, patience=500, verbose=True)
            end_time = time.time()
            during = end_time - start_time
            print('During Time:', during, 'seconds;', during/60, 'minutes;', during/60/60, 'hours;')
            torch.save(gcn.state_dict(), save_file + f'_pert{pert:.2f}_seed' + str(i) + '_checkpoint.pt')  
            # gcn.test(test_mask)
            # logits = gcn.predict()
            logits = gcn.output
...
'''
nohup python -u run_gcn.py --dataset ogbarxiv --suffix splitoset_hid256_lay3 --lr 0.01 --decay 0 --hid 256 --atk_suffix seed123 --gpu 2 > log/ogbarxiv_oset/splitoset_hid256_lay3.log 2>&1 &


nohup python -u ogbrun_gcn.py --suffix testtime --runs 1 --device 0 > log/ogbarxiv_oset/testtime.log 2>&1 &
nohup python -u run_gcn.py --dataset ogbarxiv --suffix testtime --atk_suffix seed123 --gpu 2 > log/ogbarxiv/testtime.log 2>&1 & 
'''


'''
nohup python -u run_gcn.py --dataset 12k_reddit --suffix gcn --atk_suffix seed123 --gpu 0 > log/12k_reddit/gcn.log 2>&1 & 
nohup python -u run_gcn.py --dataset 10k_ogbproducts --suffix gcn --atk_suffix seed123 --gpu 1 > log/10k_ogbproducts/gcn.log 2>&1 & 
nohup python -u run_gcn.py --dataset ogbarxiv --suffix gcn --atk_suffix seed123 --gpu 2 > log/ogbarxiv/gcn.log 2>&1 & 

nohup python -u run_gcn.py --dataset citeseer --suffix gcn --atk_suffix seed123 --gpu 2 > log/citeseer/gcn.log 2>&1 & 
nohup python -u run_gcn.py --dataset cora --suffix gcn --atk_suffix seed123 --gpu 4 > log/cora/gcn.log 2>&1 & 
nohup python -u run_gcn.py --dataset pubmed --suffix gcn --atk_suffix pre --gpu 3 > log/pubmed/gcn.log 2>&1 & 
