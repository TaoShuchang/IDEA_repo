...
def compute_nei(adj_tensor, n):
    edge_index = adj_tensor.coalesce().indices()
    nei_list = [edge_index[1][torch.where(edge_index[0] == i )[0]].cpu() for i in range(n)]
    return nei_list
...
def obtain_pert_tensor(adj, train_mask, device):
    d_train = np.array(adj.sum(0))[0][train_mask]
    tr_n = train_mask.shape[0]
    use_tr_n = int(0.01*tr_n)
    idx = np.argsort(d_train)[::-1][:use_tr_n]
    use_idx = train_mask[idx]

    tmp = train_mask.reshape(1,tr_n)
    use_idx_tmp = use_idx.reshape(1,use_tr_n)

    I_indices =  np.concatenate((use_idx_tmp.repeat(tr_n,axis=1), tmp.repeat(use_tr_n,axis=0).reshape(1, tr_n*use_tr_n)),0)
    # I_indices =  np.concatenate((use_idx_tmp.repeat(use_tr_n,axis=1), use_idx_tmp.repeat(use_tr_n,axis=0).reshape(1, use_tr_n*use_tr_n)),0)

    I_sp = sp.csr_matrix((np.ones(tr_n*use_tr_n), (I_indices[0], I_indices[1])), shape=(n,n))
    # I_sp = sp.csr_matrix((np.ones(use_tr_n*use_tr_n), (I_indices[0], I_indices[1])), shape=(n,n))
    pert_adj = I_sp - adj - adj
    graph_save_file = '../splits/perturbation/reddit_pert_10'

    np.savez(graph_save_file+ '.npz', adj_data=pert_adj.data, adj_indices=pert_adj.indices, adj_indptr=pert_adj.indptr,
            adj_shape=pert_adj.shape, labels=use_idx)
    return pert_tensor
